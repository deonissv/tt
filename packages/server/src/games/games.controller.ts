...
export class GamesController {
  ...
  @UseGuards(JwtAuthGuard)
  @Get(':code')
  get(@User() user: ValidatedUser, @Param('code') code: string): Promise<GameDto> {
    return this.gamesService.get(user, code);
  }
  ...
  update(@User() user: ValidatedUser, @Param('code') code: string, @Body() updateGameDto: UpdateGameDto) {
    return this.gamesService.update(user, code, updateGameDto);
  }
  ...
  delete(@User() user: ValidatedUser, @Param('code') code: string) {
    return this.gamesService.delete(user, code);
  }
}
