...
describe('Simulation', () => {
  beforeEach(() => {
    ...
    vi.spyOn(Loader, 'loadModel').mockImplementation(() => {
      return Promise.resolve([new Mesh('testMesh'), new Mesh('testMesh')]);
    });
  });
  ...
  //     vi.spyOn(Actor, 'fromState').mockImplementation((actorState: ActorState) => {
  //       return Promise.resolve(new Actor(actorState, new Mesh('testMesh')));
  //     });
  //     const sim = await Playground.init(initialState);
  ...
  describe('toStateSave', () => {
    it('returns complete state save', async () => {
      ...
      const sim = await Playground.init(null, initialState);
      ...
      sim['_scene'].meshes.forEach(mesh => {
        const actor = mesh.parent as Actor;
        const expectedActorState = expected.actorStates?.find(
          expectedActorState => expectedActorState.guid === actor.guid,
        );
        expect(actor.toState()).toMatchObject(expectedActorState);
      });
    });
  });
  ...
  //     const mergedState = Playground.mergeStateDelta(state, delta);
